...
import com.chen.common.utils.FileUtils;
...
import com.chen.oss.exception.OssException;
...
public class SysOssServiceImpl extends ServiceImpl<SysOssMapper, SysOss>
    ...
    public SysOssVo getVoById(Long ossId) {
        ...
        return sysOssConvert.ossToVo(oss);
    }
    ...
    public TableDataInfo<SysOssVo> queryPageList(SysOssDTO dto, PageQuery pageQuery) {
        Page<SysOss> page = baseMapper.selectPage(pageQuery.build(), this.buildQueryWrapper(dto));
        // 将OSS对象存储视图对象列表 中的url 进行外链操作
        List<SysOssVo> result = page.getRecords().parallelStream().map(oss -> {
            SysOssVo ossVo = sysOssConvert.ossToVo(oss);
            return this.matchingUrl(ossVo);
        }).collect(Collectors.toList());
        // 创建一个Page<SysOssVo>分页对象，并将所需值传入
        Page<SysOssVo> voPage = new Page<>(page.getCurrent(), page.getSize(), page.getTotal());
